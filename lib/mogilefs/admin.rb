...
class MogileFS::Admin < MogileFS::Client
  ...
  ##
  # Creates a new channel in +domain+ named +channel+.

  def create_channel(domain, channel)
    res = @backend.create_channel :domain => domain, :channel => channel
    return !res.nil?
  end

  ##
  # Removes channel +channel+ from +domain+.  Returns true if successful, false if
  # not.

  def delete_channel(domain, channel)
    res = @backend.delete_channel :domain => domain, :channel => channel
    return !res.nil?
  end
