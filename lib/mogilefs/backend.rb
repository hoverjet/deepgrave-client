...
class MogileFS::Backend
  class ConnectionLost < MogileFS::Error; end
  ...
  add_command :create_channel
  add_command :update_channel
  add_command :delete_channel

  add_command :channel_front

  add_command :have_minutes

  add_command :create_fragment
  add_command :delete_fragment
  add_command :rescreenshot_fragment
  add_command :fragment_info

  add_command :set_offset_force
  add_command :set_offset_soft
  add_command :get_paths_with_offset

  add_command :get_key_offset

  add_command :get_channel_fronts
  ...
  add_error 'no_beginning'
  add_error 'no_ending'
  add_error 'no_channel'
  add_error 'channel_not_found'
  add_error 'db_error'
  add_error 'no_frid'
  add_error 'no_fragment'
  add_error 'too_long_span'

  private unless defined? $TESTING

  def do_request_unlocked(cmd, args)
    ...
    raise MogileFS::UnreadableSocketError unless readable
    
    line = socket.gets
    raise ConnectionLost unless line
    parse_response(line)
  end
  ...
  def parse_response(line)
    if (md = line.match(/^ERR\s+(\w+)\s*(.*)/)) then
      @lasterr = md[1]
      @lasterrstr = md[2] ? url_unescape(md[2]) : nil
      raise error(@lasterr), @lasterrstr
      return nil
    end

    md = line.match(/^OK\s+\d*\s*(\S*)/)
    return url_decode(md[1]) if md
