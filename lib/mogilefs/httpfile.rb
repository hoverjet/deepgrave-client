...
class MogileFS::HTTPFile < StringIO
  ...
  attr_accessor :session, :file_to_put

  def sync_mode!(session)
  ...
  def initialize(mg, fid, klass, key, dests, content_length, allow_empty)
    super ''
    @mg = mg
    @fid = fid
    @uri = @devid = nil
  ...
  def upload(devid, uri)
    file_size = length
    if sync_mode?
      resp = file_to_put ? session.put_file(uri.to_s, file) : session.put(uri.to_s, string)
      case resp
      when 200..299
        # no op
      else
        raise BadResponseError, "HTTP response: #{resp}"
      end
    else
